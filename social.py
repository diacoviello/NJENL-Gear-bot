...
Rank system (tied to actual Telegram group roles):
  ...
  Soldier    = regular member promoted by a Capo or Underboss via /promote
  Associate  = everyone else

  /rank            — show your own rank
  /promote         — reply to a message to promote that user to Soldier
  /family          — show the full roster
"""
...
def _group_only(func):
    async def wrapper(update: Update, context: ContextTypes.DEFAULT_TYPE):
        if update.effective_chat.type == "private":
            await update.message.reply_text("This is a group thing. Take it to the Family chat.")
            return
...
async def unrat_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if len(rats) == before:
        await update.message.reply_text(f"{username} wasn't on the list to begin with.")
        return
    storage.set(f"rats:{chat_id}", rats)
...
async def rats_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    rats = storage.get(f"rats:{chat_id}", [])
    if not rats:
        await update.message.reply_text("🐀 The list is clean. Nobody's been calling us rats.")
        return
    lines = ["🐀 *Known Rats:*\n"]
    for r in rats:
...
async def promote_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not target_id:
        await update.message.reply_text(
            "Reply to their message to promote them. `/promote` (as a reply)", parse_mode="Markdown"
        )
        return
    target_rank = await _get_rank(context, chat_id, target_id, storage)
    if target_rank != "Associate":
        await update.message.reply_text(f"{target_username} is already {target_rank}. Nothing to do here.")
        return
    soldiers = storage.get(f"soldiers:{chat_id}", [])
...
async def family_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        await update.message.reply_text("Can't pull the family tree right now. Try again.")
        return
    underbosses, capos = [], []
    for admin in admins:
    ...
    soldier_lines = [
        ...
        for s in soldiers if isinstance(s, dict) and s.get("username")
    ]
    lines = ["🔷 *The Family:*\n"]
