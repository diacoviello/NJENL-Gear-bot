...
Registered in bot.py as a repeating job (every hour).
No message is posted — runs completely silently.
"""
...
async def expire_old_entries(context: ContextTypes.DEFAULT_TYPE):
    ...
    for cfg in FLOWS.values():
        count = storage.expire_entries(cfg["store_key"], cfg["status_default"], days=EXPIRY_DAYS)
        total += count
    if total:
